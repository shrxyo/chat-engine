...
This is a full-stack, production-grade real-time chat platform — think Slack or Discord — built as an ambitious portfolio project designed to demonstrate senior-level engineering skills. The core system handles real-time messaging via WebSockets with sub-100ms latency, and is designed to scale horizontally across multiple gateway instances using Redis Pub/Sub as a fanout backbone. The tech stack spans Go or Python for the WebSocket gateway, FastAPI for the HTTP API, Next.js 15 (App Router) with TypeScript for the frontend, PostgreSQL with pgvector for persistent storage and semantic search, and Redis for pub/sub, presence tracking, and caching.
...
The project is structured across 12 epics and ~45 stories, designed to be built incrementally over a couple of weeks part-time. Each phase ends with a meaningful demo, and the scope deliberately builds toward a resume bullet that can be discussed in depth during interviews — covering horizontal scaling (Redis Pub/Sub fan-out), AI/ML integration (RAG, pgvector), observability (OpenTelemetry, Prometheus, Grafana), CI/CD (GitHub Actions, Fly.io preview deploys), and PWA/offline support. The guiding philosophy is "interview value over feature count" — depth in the right places rather than breadth.
